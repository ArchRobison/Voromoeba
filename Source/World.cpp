...
void World::initialize( NimblePixMap& window ) {
    ...
    memset( numOrange, 0, sizeof(numOrange) );
...
static void DrawBackground( NimblePixMap& window, CompoundRegion& region ) {
    if( !region.empty() ) {
        ...
        a = PondBackground.assignAnts(a,World::viewTransform); 
...
static void DrawPondGroup( NimblePixMap& window, CompoundRegion& region, size_t first, size_t last ) {
    ...
    if( Finale::isRunning() )
        ...
        a = Finale::assignAnts(a,window,Self.pos,PondSet[Self.pondIndex].center()); 
    a = Missiles::assignAnts(a, first, last);
    for( size_t k=first; k<last; ++k ) 
        if( PondSet[k].isDark() )
            a = PondSet[k].assignDarkAnts(a,World::viewTransform);
        else
            a = PondSet[k].assignAnts(a,World::viewTransform); 
...
static bool IsBumpable( BeetleKind selfOrMissile, BeetleKind other ) {
    Assert( selfOrMissile==BK_SELF || selfOrMissile==BK_MISSILE );
    return other==BK_PREDATOR || other==BK_SWEETIE || other==BK_ORANGE || selfOrMissile==BK_SELF && other==BK_PLANT;
}
    ...
    for( Neighbor* s = buffer; s<e; ++s ) {
        ...
        if( localIndex<p->safeSize() ) {
            ...
            if( SegmentOverlapsPondOrBridge(kMin,kMax,a,c) ) {
                ...
                if( other.kind==BK_WATER ) {
                    float r = 1.0f;
                    if( b.kind==BK_MISSILE ) {
