...
class OutlinedColor {
    ...
    static const unsigned interiorMask = 0xFFFFFF;
    //! Maximum number of distinct exterior colors 
    static const unsigned exteriorNumColorMax = 255;
    static unsigned exteriorColorCount;
    static NimblePixel exteriorTable[exteriorNumColorMax+1];
public:
    //! Index of an exterior color.  Zero means "no exterior color".
    typedef unsigned char exteriorColor;
...
public:
    ...
    static void start() {
        array[0].id = 0;    // Initialize sentinel
        ptr = &array[1];
        idCount = 0;
    }
