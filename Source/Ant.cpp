...
static Ant AntArray[2][N_ANT_MAX];
static Point To[N_ANT_MAX];
static Point From[N_ANT_MAX];
static Ant* BufferFirst;
static Ant* BufferPtr;
...
/** Always 0 or 1 */
static unsigned char CurrentHalf;
bool ShowAnts;
...
static void SetPoints( NimblePixMap& window, const Ant* first, const Ant* last, Point outer[] ) {
    Assert( first<=last );
    Point center = Point(window.width()/2,window.height()/2);
    float r = 0.6*std::sqrt(Dist2(window.width(),window.height()));
    Point* o = outer;
    for( const Ant* a=first; a!=last; ++a ) {
        Point p = *a - center;
        *o++ = center+Polar(r,RandomFloat(1)+atan2(p.y,p.x)); 
    }
}
...
static Ant* AntCutCompose( NimblePixMap& window, Ant* antLast ) {
    ...
    float t = 1.0f*(globalTime-baseTime);
#if 1   
    ...
    if( t<1.0f ) {
        ...
        for( Ant* a = BufferFirst+1; a!=antLast; ++a, ++p ) 
            static_cast<Point&>(*a) = f**a + (1-f)**p;
    }
#endif
#if 1
    if( t<2.0f ) {
        // Interpolate old buffer with to 
        float f = Min(1.0f,2.0f-t);
        const Point* p = To;
        Ant* a = antLast;
