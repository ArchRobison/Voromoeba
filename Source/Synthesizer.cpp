...
class Player {
    void output(float* dst, const float* src, float vol, uint32_t n) {
...
public:
    ...
    uint32_t postSource;
    static const uint32_t delayBufSize = 64;
    float delayBuf[delayBufSize];
    static const uint32_t chunkMaxSize = 1024;
    //! Accumulate into left and right.  Return false if done.
    bool update(float* left, float* right, uint32_t n);
...
bool Player::update(float* left, float* right, uint32_t n) {
    Assert(0<n);
    ...
    uint32_t d = Max(delay[0], delay[1]);
    ...
    if (postSource>0) {
        ...
        const uint32_t m = source->update(buf+d, n);
        if (m<n) {
...
void Play(Source* src, float volume, float x, float y) {
    ...
    p->source = src;
    src->player = p;
    p->volume[0] = volume*std::cos(std::atan2(y, -x)/2);
    p->volume[1] = volume*std::cos(std::atan2(y, x)/2);
...
uint32_t SimpleSource::update(float* acc, uint32_t n) {
...
uint32_t DynamicSource::update(float* acc, uint32_t n) {
    uint32_t requested = n;
...
uint32_t MidiSource::update(float* acc, uint32_t n) {
