...
void Pond::initialize( NimblePixMap& window, size_t n, PondOptions pondOptions, size_t numOrange ) {
    ...
    initializeColorAndSound( window, sweetieEnd, predatorEnd, NimbleColor(0x80,0,0), NimbleColor(0xC0,0x00,0x00), BK_PREDATOR, exPredator );
    // Put food into pond
    initializeColorAndSound( window, predatorEnd, foodEnd, NimbleColor(0,0xC0,30), NimbleColor(0,0xFF,0x40), BK_PLANT, exPlant );
    // Put oranges into pond
    initializeColorAndSound( window, foodEnd, orangeEnd, NimbleColor(0xFF,0xB0,0), NimbleColor(0xFF,0xD0,0), BK_ORANGE, exOrange );
    // Put water into pond
    initializeColorAndSound( window, orangeEnd, size(), NimbleColor(0,0,0x40), NimbleColor(0,0,0xFF), BK_WATER, exWater );
    ...
    for( size_t k=0; k<size(); ++k ) {
        ...
        switch( b.kind ) {
            case BK_PREDATOR: isMoving = PO_PredatorMoves; break;
            case BK_PLANT:    isMoving = PO_FoodMoves; break;
            case BK_WATER:    isMoving = PO_WaterMoves; break;
        }
        if( myPondOptions & isMoving ) {
            float theta = RandomAngle();
            float speed;
            if( (myPondOptions & PO_Zinger) && b.kind==BK_PREDATOR )
                speed = 0.15f*radius()*(RandomFloat(3)+1);
            else if( (myPondOptions & PO_Crystalline) && b.kind==BK_WATER ) 
...
void Pond::update( float dt ) {
    if( safeSize()<size() ) {
        ...
        if( back().kind==BK_PREDATOR )
            --myPredatorCount;
        popBack();
    }
    for( size_t i=0; i<size(); ++i ) {  
        Beetle& b = (*this)[i];
        Assert( b.color.hasExterior() || !(b.kind==BK_PREDATOR||b.kind==BK_PLANT||b.kind==BK_ORANGE) );
        if( b.kind==BK_PREDATOR ) {
            b.color.setInterior( PredatorColorWobble(b.orbit) );
            Assert( b.color.hasExterior() );
        } else if( b.kind==BK_SWEETIE ) {
            b.color.setInterior( SelfType::colorWobble(b.orbit) );
            Assert( b.color.hasExterior() );
        } else if( b.kind==BK_WATER ) {
            if( myPondOptions & PO_Crystalline ) 
                continue;
        }
        float s = dt;
        ...
        for( int count=20; epsilon<s && epsilon<Dist2(b.vel) && count>0; --count ) {
            ...
            if( s<deltaS ) {
                ...
                b.pos += deltaS*b.vel;
                s -= deltaS;
                if( (b.kind==BK_PREDATOR) && (myPondOptions & PO_Zinger) && Dist2(Self.pos,center())<radiusSquared() ) {
