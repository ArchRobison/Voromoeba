...
Outline::segment* Outline::sortIntoBins() {
    static unsigned binCount[nIdMax];
    ...
    for( const segment* s = array; s<ptr; ++s ) {
        Assert( sorted<=binPtr[s->id] && binPtr[s->id]<sorted+nSegmentMax );
        *binPtr[s->id]++ = *s;
    }
...
void Outline::finishAndDraw( NimblePixMap& window ) {
    ...
    for( const segment* s = sorted+1; s<sortedEnd; ++s ) {
        ...
        int x0 = Max<int>( s->left, 0 );
        int x1 = Min<int>( s->right, window.width() );
